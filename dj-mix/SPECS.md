...
- 2026-08-15 : la barre de recherche est maintenant affichée directement sur l'écran initial de `karaoke-playlist/index.html` (plus besoin du bouton flottant 🔍 pour l'ouvrir, supprimé avec l'overlay de recherche associé) ; seule la file d'attente reste dans un overlay accessible en tapant la barre "en cours". `karaoke-playlist/player.html` désactive les sous-titres YouTube (`cc_load_policy:0` + `unloadModule('captions')` appelé à chaque chargement de vidéo, car `cc_load_policy` seul ne suffit pas si l'utilisateur a forcé les sous-titres dans ses réglages YouTube).
- 2026-08-15 : suppression de tous les textes d'aide explicatifs (hints) dans `karaoke-playlist/` — le sous-texte sous la barre de recherche, le paragraphe sous "Démarrer" et celui sous "En attente" sur l'écran vidéo. Seuls les libellés d'état essentiels (ex. "File d'attente vide.") sont conservés.
- 2026-08-15 : dans `karaoke-playlist/index.html`, l'ajout d'une chanson (bouton "Ajouter" sur un résultat de recherche) vide maintenant le champ de recherche, les résultats et le statut (`resetSearch()`) au lieu d'afficher "Ajouté ✓" en désactivant temporairement le bouton — l'écran revient directement à l'état de recherche vierge.
- 2026-08-15 : correction d'un bug où le bouton "Passer" pouvait vider toute la file d'attente. Cause : `player.html` retirait la tête de file sans condition (`dequeueCurrent`) quand une vidéo se terminait ; si l'écran de contrôle avait déjà retiré cette même chanson via "Passer" juste avant, un ENDED tardif du lecteur (course connue avec `loadVideoById` côté API YouTube) retirait une deuxième chanson, pouvant vider une file de 2 titres. Remplacé par `KaraokeState.removeIfCurrent(id)` : la tête de file n'est retirée que si son `id` correspond encore à la chanson que le lecteur vient de terminer, rendant l'opération idempotente entre les deux écrans.
- 2026-08-15 : correction d'un bug d'affichage dans `karaoke-playlist/index.html` — l'API YouTube Data v3 renvoie `snippet.title`/`snippet.channelTitle` HTML-échappés (ex: "Pour que tu m&#39;aimes encore"), et comme le rendu utilise `textContent` (volontairement, pas d'innerHTML), ces entités s'affichaient telles quelles au lieu d'être décodées. Ajout de `decodeHtmlEntities()` (décodage sûr via un `<textarea>` temporaire, jamais interprété comme balises) appliqué aux titres/chaînes dès la lecture des résultats de recherche, avant affichage ET avant stockage dans la file (`KaraokeState.addTrack`).
- 2026-08-15 : certaines vidéos (ex: chaîne Sing King) ont l'intégration désactivée par leur propriétaire — injouables dans `player.html`. `karaoke-playlist/index.html` appelle maintenant `videos.list?part=status` après chaque recherche pour exclure des résultats les vidéos avec `embeddable:false`, avant même qu'elles puissent être ajoutées à la file.
- 2026-08-15 : le filet de sécurité `armStallTimer`/`STALL_TIMEOUT_MS` ajouté juste avant dans `player.html` (skip auto si la lecture ne démarre pas en 15s) sautait en fait TOUTES les chansons — le timer n'était jamais annulé au passage en `PLAYING`, donc il se déclenchait systématiquement 15s après le début de n'importe quelle vidéo, y compris celles qui jouaient normalement. Supprimé entièrement à la demande de l'utilisateur plutôt que réparé : le filtrage `embeddable` à la recherche couvre déjà la cause réelle, et `onError` reste le filet de sécurité pour le reste.
- 2026-08-15 : le stall timer a été restauré à la demande, cette fois avec `onPlayerStateChange` qui appelle `clearStallTimer()` sur l'état `PLAYING` (pas seulement `ENDED`) — vérifié par simulation qu'une vidéo qui démarre normalement n'est plus jamais sautée, et qu'une vidéo bloquée l'est toujours après 15s.
- 2026-08-15 : correction d'un `ReferenceError: KaraokeState is not defined` — `<script src="state.js">` et `window.open('player.html', ...)` étaient en chemins relatifs. Si la page est ouverte sans slash final (`/karaoke-playlist` au lieu de `/karaoke-playlist/`, ce que fait le serveur local `serve` de `npm run start:local` sans redirection, contrairement à `python -m http.server` qui redirige en 301), le navigateur résout ces chemins relatifs contre la racine du site au lieu du dossier `karaoke-playlist/`. Remplacés par des chemins absolus (`/karaoke-playlist/state.js`, `/karaoke-playlist/player.html`) dans `index.html` et `player.html`, insensibles à la présence du slash final.
- 2026-08-18 : refonte complète de `karaoke-playlist/` pour utiliser le serveur dj-mix (nouveaux endpoints `POST /api/video/download` et `GET /api/video/stream?cachePath=...`, cf. `swagger.json` — port 3004 pour `downloadServer.js`, port 3002 pour `audioCdnServer.js`) au lieu de l'IFrame Player YouTube et de la clé API YouTube Data v3 côté client. Changements :
  - `karaoke-playlist/server-config.js` (nouveau) : config partagée (URL API, token, URL CDN vidéo — dérivée de l'URL API si vide) via localStorage, avec détection du blocage "contenu mixte" (page HTTPS → serveur local http://), repris du pattern `lib/downloaderConfig.js` de dj-mix.
  - `index.html` : le panneau clé API YouTube est remplacé par un panneau "Réglages serveur" (URL API/token/URL CDN + bouton Test sur `/health`). La recherche appelle `GET /api/search?term=...` (recherche + matching IA côté serveur, plus besoin de clé API ni du filtrage `embeddable` — yt-dlp ne se soucie pas des restrictions d'intégration). "Ajouter" déclenche `POST /api/video/download` (synchrone, orchestration-only, peut être lent sur un MISS) avant d'ajouter le `cachePath` retourné à la file — bouton en état "Téléchargement…" pendant l'appel.
  - `player.html` : l'IFrame Player YouTube est remplacé par un `<video>` HTML natif, `src` pointant vers `GET {cdnUrl}/api/video/stream?cachePath=...&token=...`. Les events `playing`/`ended`/`error` remplacent `onStateChange`/`onError` ; le filet de sécurité `armStallTimer` (15s, désarmé sur `playing`) est conservé à l'identique côté logique.
  - `state.js` : le champ de piste `videoId` devient `cachePath` ; `STORAGE_KEY` bump à `_v3` (les anciennes entrées en cache ne sont plus compatibles).
  - Le serveur local de dev (port 3000) ne répondait plus (connection reset sur toute requête HTTP, y compris `/swagger.json`) au moment de cette session — le contrat API exact a été obtenu via le `swagger.json` collé directement par l'utilisateur plutôt que consulté en direct comme le veut la consigne habituelle.
- 2026-08-18 : refonte des réglages serveur de `karaoke-playlist/index.html` pour suivre le pattern de `dj-mix/index.html` (`lib/downloaderConfig.js`) : suppression du champ "URL du CDN vidéo" (dj-mix ne l'expose pas non plus — la CDN est toujours dérivée de l'URL API, `getCdnUrl()` simplifié dans `server-config.js`), et remplacement du panneau réglages en overlay plein écran (ouvert via une icône ⚙️ dans l'en-tête) par une vraie barre de navigation basse (`.tab-bar`, 2 onglets "Recherche"/"Réglages") — même structure `data-tab`/`switchTab()` que le `.tab-bar` de dj-mix. La barre "en cours de lecture" (Karafun) passe au-dessus de cette nouvelle barre de nav, façon mini-lecteur Spotify. Bascule automatique sur l'onglet Réglages tant que le serveur n'est pas configuré.
- 2026-08-18 : `karaoke-playlist/index.html` bascule de `GET /api/search` (endpoint audio générique, `tracks.results[]`, champs `trackName`/`artistName`/`youtubeUrl`/`youtubeTitle`) vers le nouvel endpoint dédié `GET /api/video/search?term=...&limit=...` (pas de paramètre `artist`, `results[]` à la racine, champs `videoTitle`/`channelName`/`videoUrl`/`durationSeconds`/`isLocal`/`cachePath`). Ajout du court-circuit `isLocal:true` : si le résultat de recherche porte déjà un `cachePath`, "Ajouter" saute entièrement l'appel à `POST /api/video/download` (bouton libellé "Ajouter" simple) ; sinon le bouton dit "Ajouter (télécharger)" et le corps de la requête se limite à `{ youtubeUrl }` (plus de `trackName`/`artistName`/`youtubeTitle`, ce endpoint n'a pas de matching catalogue iTunes/Deezer comme l'audio). Ajout de `formatDuration()` pour convertir `durationSeconds` en `m:ss` (remplace le `durationLabel` string qui n'existe plus sur ce endpoint).
- 2026-08-18 : ajout d'un flux invités (QR code) à `karaoke-playlist/` synchronisé via Firestore, pour laisser les gens ajouter des chansons depuis leur téléphone pendant la soirée. Contrainte de conception : le serveur dj-mix n'est joignable que depuis le LAN de l'hôte (ex. `https://192.168.8.149:8443`), donc une Cloud Function (infra Google Cloud) ne peut PAS l'appeler directement — seul l'écran de contrôle (sur le LAN, déjà configuré avec le token API) le peut. Architecture retenue :
  - `player.html` affiche en permanence un QR code (lib `qrcodejs` via CDN, panneau discret en bas à droite) encodant `guest.html?session=<id>` — `id` vient de `state.js#getSessionId()` (UUID généré une fois, partagé avec l'écran de contrôle via le même localStorage).
  - `guest.html` (nouvelle page mobile) : un invité tape juste "Titre - Artiste" en texte libre, sans recherche/prévisualisation (impossible sans accès direct à dj-mix) ; le formulaire appelle la Cloud Function callable `submitSongRequest` (jamais d'écriture Firestore directe depuis le navigateur invité) et suit ensuite le statut de sa demande en temps réel (`pending` → `done`/`failed`).
  - `functions/index.js` (Cloud Function `submitSongRequest`, 2ᵉ génération, SDK Admin) : valide `sessionId`/`query`, plafonne à 30 demandes `pending` par session (garde-fou anti-abus, pas d'App Check), puis crée `sessions/{sessionId}/requests/{id}` avec `status:'pending'`.
  - `index.html` (écran de contrôle) : `<script type="module">` séparé qui écoute `sessions/{sessionId}/requests` en temps réel (`watchPendingRequests`), appelle `POST /api/video/download` avec `{trackName: query, searchQuery: query}` pour chaque demande en attente (dédupliquée via un `Set` local le temps du téléchargement, potentiellement long), ajoute le résultat à la file locale comme un ajout manuel, puis marque la demande traitée (`markRequestDone` = suppression du doc) ou échouée (`markRequestFailed` = `status:'failed'` + message).
  - `firestore.rules` : `create` interdit aux clients (uniquement la Cloud Function, SDK Admin) sur `sessions/*/requests/*` ; `read`/`update`/`delete` ouverts (le `sessionId`, UUID aléatoire non déductible, sert de capacité d'accès — modèle volontairement simple pour une appli de soirée entre invités de confiance).
  - `firebase-config.js` contient des valeurs placeholder (`REMPLACE_MOI`) à remplacer par la config du projet Firebase réel de l'utilisateur avant que quoi que ce soit ne fonctionne.
  - Vérifié : QR code décodé pixel-par-pixel via jsQR (URL exacte confirmée), toutes les pages chargent sans erreur JS avec la config Firebase placeholder. Le flux Firestore de bout en bout (invité → Cloud Function → écran de contrôle → dj-mix) n'a pas pu être testé faute de vrai projet Firebase configuré.
- 2026-08-18 : `functions/`, `firebase.json`, `.firebaserc` et `firestore.rules` ont été déplacés hors de ce repo (front-end statique) vers le repo backend `Spotify-mp3-downloader` (`c:/Users/Niafrond/dev/Spotify-mp3-downloader`, racine du dépôt, à côté de `downloadServer.js`/`audioCdnServer.js` — voir sa section SPECS.md "Cloud Function invités karaoke-playlist (Firestore)") — le code de la Cloud Function vit désormais côté back plutôt que dans le repo GitHub Pages, même si elle continue de se déployer sur l'infra Firebase (pas le serveur systemd/nginx de ce repo backend). Ne restent dans `karaoke-playlist/` que les pièces réellement front-end : `firebase-config.js` (config client), `firebase-sync.js` (SDK client Firestore/Functions), `guest.html`, et l'intégration QR code (`player.html`)/écoute des demandes (`index.html`). `firebase deploy --only functions,firestore:rules` se lance désormais depuis `Spotify-mp3-downloader/`, pas depuis ce repo.
- 2026-08-18 : Cloud Function déployée (`https://us-central1-karaoke-506217.cloudfunctions.net/submitSongRequest`) — `firebase-config.js` mis à jour avec le vrai `projectId` (`karaoke-506217`, `authDomain` dérivé ; `storageBucket` volontairement omis, l'app n'utilise pas Firebase Storage) ; `apiKey`/`messagingSenderId`/`appId` restent en placeholder, à fournir pour que la connexion Firestore (temps réel) fonctionne. `firebase-sync.js#submitSongRequest` appelle désormais la fonction en `fetch` HTTP direct (`{ data: {...} }` → `{ result: {...} } | { error: {...} }`) plutôt que via le SDK client Functions (`getFunctions`/`httpsCallable`, retiré) — un import de moins, et confirmé fonctionner en prod par l'utilisateur. Corrigé au passage un bug introduit par ce changement : `watchRequestStatus` (statut de la demande de l'invité) utilisait `db` sans jamais appeler `ensureInitialized()` — n'importe quelle autre fonction du module le faisait auparavant en effet de bord (dont l'ancien `submitSongRequest` via le SDK), donc l'appeler dans `watchRequestStatus` seul rendait le bug invisible jusqu'à ce que ce chemin s'exécute isolément. Repéré par un test simulant le flux `guest.html` (payload/URL/liste de demandes vérifiés) avant déploiement réel.
- 2026-08-24 : refonte complète de l'architecture karaoke-playlist à la demande de l'utilisateur — deux écrans au lieu de trois, rôles inversés par rapport à la version précédente :
  - **`index.html`** (renommé depuis `player.html`, l'ancien `index.html` "écran de contrôle" est supprimé) devient l'**écran maître** : plein écran vidéo + QR code visible en permanence (bas-droite, avec le sessionId affiché en clair en dessous pour le copier dans `KARAOKE_SESSION_ID` côté serveur), **aucun contrôle** — ni recherche, ni ajout, ni gestion de file. Garde un petit panneau réglages discret (icône ⚙️ en haut-gauche) pour la seule config encore nécessaire côté navigateur : URL/token du serveur, utilisés uniquement pour construire les URLs de streaming (`GET /api/video/stream`) — plus aucun appel de recherche/téléchargement direct depuis un navigateur, tout est relayé côté back (voir `Spotify-mp3-downloader/SPECS.md` > "Cloud Functions invités karaoke-playlist (Firestore)"). Script fusionné en un seul `<script type="module">` (plus de pont `window.X` entre un script classique et un module).
  - **`guest.html`** (atteint en scannant le QR) devient l'écran complet : recherche avec vrais résultats (relayée via la nouvelle Cloud Function `submitSearchQuery` + `sessions/{id}/searchQueries`, traitée côté back par `karaokeRequestsWatcher.js` qui appelle `GET /api/video/search`), choix d'un résultat précis pour l'ajout (`submitAddRequest` → `submitSongRequest` avec `youtubeUrl` cette fois, plus un texte en aveugle), et **file d'attente visible et modifiable par tout le monde** ("Passer"/"Retirer" = suppression Firestore directe côté client, aucune Cloud Function nécessaire pour ça).
  - **`state.js`** réduit à `getSessionId()` uniquement — la file d'attente ne vit plus du tout en localStorage/BroadcastChannel (un seul PC) mais dans Firestore (`sessions/{id}/queue`), partagée entre l'écran maître et tous les invités. `firebase-sync.js` étendu avec `watchQueue`/`removeQueueItem`/`submitSearchQuery`/`watchSearchQuery`/`submitAddRequest`/`watchAddRequest`.
  - Changement côté back nécessaire pour ce nouveau flux "recherche puis choix" (une autre session travaillait déjà sur ce repo en parallèle — `karaokeRequestsWatcher.js`, `terraform/`, `scripts/test-karaoke-flow.js` existaient déjà pour l'ancien flux "texte en aveugle" avant cette session) : voir `Spotify-mp3-downloader/SPECS.md` pour le détail (nouvelle Cloud Function `submitSearchQuery`, nouvelle sous-collection `queue` écrite par le watcher, deuxième boucle de polling, entrées Terraform correspondantes). Backend vérifié par `node --check` (Node 22 via WSL/nvm) et `terraform validate` (tous deux OK) ; front vérifié par chargement sans erreur JS des deux pages + interception des appels `fetch` vers les Cloud Functions (URL et payload corrects) — le reste (résultats de recherche affichés, file en temps réel) dépend de Firestore réel et n'a pas pu être testé de bout en bout avant que les nouvelles règles/index/fonctions ne soient déployés (`terraform apply`, pas exécuté par Claude).
- 2026-08-29 : suppression de tout accès Firebase direct depuis le front (à la demande explicite de l'utilisateur — "l'app n'a aucune raison d'avoir accès à firebase, tout passe par la cloud function"). `firebase-config.js` et `firebase-sync.js` supprimés ; remplacés par `karaoke-api.js`, qui n'importe aucun SDK Firebase — seulement des `fetch()` vers les Cloud Functions (URL construite en dur : `https://us-central1-karaoke-506217.cloudfunctions.net/<nom>`). Les listeners Firestore temps réel (`onSnapshot`) deviennent du polling (`getQueue` toutes les 3s, `getSearchQuery`/`getRequestStatus` toutes les 1.5s, arrêt automatique du polling une fois `status` à `'done'`/`'failed'`) contre quatre nouvelles Cloud Functions côté back (voir `Spotify-mp3-downloader/SPECS.md`). `index.html`/`guest.html` n'ont eu à changer que leur chemin d'import (`firebase-sync.js` → `karaoke-api.js`), la forme des fonctions exportées (`watchQueue`, `submitSearchQuery`, etc.) étant restée identique. Vérifié : chargement des deux pages sans référence Firebase/Firestore dans la console, et flux recherche→résultats simulé de bout en bout (polling pending→done, arrêt automatique, rendu des résultats).
...
- **SPEC-1.3.6.6** **BUG CORRIGÉ LE 2026-08-01 — le mode `short_reverse` et le FX manuel/auto `backspin` sonnaient comme une saccade au lieu d'un reverse fluide.** Cause : les deux faisaient entendre le morceau qui continue de jouer normalement en avant, entrecoupé de sauts durs et répétés de `HTMLAudioElement.currentTime` en arrière — `short_reverse` (`#runTransitionMode`, tick à `30 ms`) reculait de `45 ms` toutes les `30 ms` tant que `progress < 0.18` ; le FX `backspin` (`triggerBackspinFx`, `lib/djFxController.js`) chaînait `8` seeks de `90 ms` toutes les `66 ms` via `setTimeout`. Chaque seek étant instantané (pas de fondu), le résultat était une série de clics/à-coups plutôt que la sensation continue d'un morceau qui recule. Remplacé par un **vrai grain audio inversé** (`lib/reverseEngine.js`, nouveau module) : `prepareReversedWindow(ctx, url, anchorSec, windowSec)` décode transitoirement la piste déjà locale (`blob:`, même mécanique transitoire que `LoopMorphEngine#prepare`, SPEC-1.3.8.24) et ne retient qu'une petite fenêtre juste avant `anchorSec`, avec les échantillons de chaque canal **inversés dans l'ordre** — jouer ce buffer en avant sonne donc exactement comme la source jouée à l'envers. `ReverseGrainEngine#run` programme un unique `AudioBufferSourceNode` (fondu d'entrée/sortie de `12 ms` sur son propre `GainNode`, click-free) à travers le même bus Web Audio que la lecture normale (`sourceBus` / `getDeckInputBus`, donc la même chaîne echo/distortion/filtre).
- **SPEC-1.3.6.7** `DJPlayer#playReverseGrain(deck, { durationMs })` (nouvelle méthode publique, `player.js`) orchestre le tout : décode la fenêtre `durationMs` précédant la position courante de la platine, coupe la contribution de l'`HTMLAudioElement` au Web Audio graph (`SimpleMixFeatures#setDeckElementGain(deck, 0)` — l'élément continue de jouer, inaudible, sans être mis en pause, comme SPEC-1.3.8.30 pour beat_repeat), joue le grain inversé pendant sa durée réelle, puis **repositionne exactement** la platine à la position d'ancrage (`audio.currentTime = anchorSec`) et restaure `setDeckElementGain(deck, 1)`. Un "reverse blip" auto-contenu qui ne modifie donc jamais la position réelle de lecture ni la timing math des autres transitions/anchors — seulement une texture sonore superposée. No-op sûr (retourne immédiatement, sans lever) si `mixFeatures`/`AudioContext` est indisponible, la platine n'a pas de source chargée, la position est ≤ `50 ms`, ou le décodage échoue — et no-op si la platine a chargé une autre piste pendant que le décodage était en vol (comparaison de l'URL avant/après `await`).
- **SPEC-1.3.6.8** GIVEN le mode `short_reverse` — WHEN la transition démarre — THEN `playReverseGrain` est déclenché **une seule fois**, en tête de `#runTransitionMode` (pas réévalué à chaque tick comme l'ancien hack), avec `durationMs = clamp(crossfadeDuration × 0.18, 180, 600)` — la fenêtre correspond à celle que la courbe de volume `short_reverse` (`#computeTransitionLevels`, wobble sinusoïdal) cible déjà pour le deck sortant. Fire-and-forget : la boucle de tick de la transition (volume, playback rate des deux platines) continue en parallèle, indépendamment de la promesse de `playReverseGrain`.
- **SPEC-1.3.6.9** GIVEN le FX `backspin` (manuel ou Auto DJ FX) — WHEN `triggerBackspinFx(deck)` est appelé — THEN il combine `playReverseGrain(deck, { durationMs: 640 })` avec la même envolée de pitch qu'avant (`applyTemporaryDeckPlaybackRate(deck, 1.18, 640)` — inchangée, c'est elle qui donne la sensation du vinyle qui rattrape sa vitesse au relâchement) ; l'ancienne boucle de `8` seeks est entièrement supprimée. Toujours un no-op si la position de la platine est `≤ 200 ms` (garde préexistante, inchangée).
...
- **SPEC-2.5.5** Chaque item affiche un bouton `.queue-refresh-mix-btn` (« Rafraîchir la piste ») — WHEN cliqué — THEN `_refreshQueueTrack(item, btn)` (`main.js`) appelle `refreshQueueTrack` (`lib/trackRefresh.js`), qui délègue à `redownloadTrackAndWait(item)` (`lib/audioSourceManager.js`) : `POST {apiUrl}/api/cache/redownload-tracks` avec `{tracks:[{trackName, artistName, cachePath}]}` pour forcer un re-téléchargement serveur frais depuis YouTube en conservant les autres métadonnées de l'entrée (artwork, genre, date de sortie...). Cet endpoint ne fait qu'empiler un job en tâche de fond (`{queued}`) : `redownloadTrackAndWait` sonde ensuite `GET {apiUrl}/api/cache/files` (toutes les `4s`, jusqu'à `5min`) jusqu'à ce que l'entrée correspondante (par `cachePath`, sinon par `trackName`+`artistName` normalisés) expose un `cachedAt` différent de sa valeur avant l'appel. Le bouton reçoit la classe `is-checking` (spinner) pendant toute l'attente. GIVEN le job se termine et le fichier frais est confirmé — THEN un toast `Piste rafraîchie : <nom>` est affiché ; GIVEN le serveur ne file rien (`queued: 0`) ou que le polling expire sans changement de `cachedAt` — THEN un toast d'erreur `Échec du rafraîchissement` est affiché (`Erreur réseau` en cas d'exception). Ce bouton ne rafraîchit plus les données de mix (`autoModeManager.refreshMixData`, cf. ancien SPEC-2.5.5) — cette capacité reste disponible sur `autoModeManager` mais n'est plus câblée sur ce bouton.
- **SPEC-2.5.6** GIVEN le redownload est confirmé (cachedAt changé) — THEN `refreshQueueTrack` évince les sources locales (`evictTrackSource`, blob mémoire + IndexedDB) et vide `item.persistedSourceUrl`, PUIS résout une source fraîche via `ensureLocalSource(item, { forceFreshResolve: true })`, qui ignore explicitement les raccourcis locaux (`cachePath`, blob en mémoire, blob persistant IndexedDB, lookup `trackPathDb`) pour forcer une résolution API fraîche (`POST /api/download`) au lieu de rejouer une source potentiellement obsolète. GIVEN le redownload échoue ou expire — THEN aucune source locale n'est touchée (la piste continue de jouer depuis sa source existante). GIVEN le morceau rafraîchi est toujours celui en cours de lecture (`uiState.currentTrackId === item.id && uiState.isPlaying`) une fois le redownload terminé — THEN `startPlaybackForIndex(uiState.currentIndex, 'play')` est appelé pour relancer la piste avec le fichier fraîchement téléchargé.
...
- **SPEC-9.3.4** GIVEN un artwork d'item relay (`queue`, `filRouge`, `filRougeNext`, `relayIncoming`) — WHEN l'état est diffusé par `buildRelayStateSnapshot()` OU rendu par `relay.js` — THEN toute URL `/api/artwork?...` ou URL relative est normalisée vers une URL absolue du CDN configuré (avec `token` si présent) avant d'être envoyée/affichée, et toute URL `blob:` est supprimée pour éviter que le relais dépende du maître pour l'image ; l'affichage relais charge donc l'artwork directement depuis le CDN.
- **SPEC-9.3.4.1** GIVEN un item de `queue`/`filRouge`/`filRougeNext` dont `item.artUrl` a été réécrit en `blob:` local côté maître (mise en cache d'artwork pour l'affichage du maître, `fetchAndStoreArtworkForItem`/`fetchFilRougeArtwork`) — WHEN `buildRelayStateSnapshot()` construit l'état diffusé — THEN, puisque `resolveArtworkUrlForRelay` renvoie `''` pour une URL `blob:`, l'artwork est repris depuis `lib/artworkUrlCache.js` (`getArtworkUrl(name, artist)`, dernière URL distante connue pour ce titre) plutôt que laissée vide. Corollaire : `fetchAndStoreArtworkForItem`/`fetchFilRougeArtwork` mémorisent désormais systématiquement l'URL distante via `setArtworkUrl()` dès qu'elle est connue, même quand un blob local existe déjà simultanément (avant ce fix, `setArtworkUrl` n'était appelé que dans la branche mutuellement exclusive "pas de blob local", donc jamais atteint pour un morceau déjà en cache — le cas le plus fréquent une fois la lecture commencée). Bug observé avant fix : jaquette visible pendant le téléchargement/l'ajout à la file (URL distante encore intacte) puis disparaissant dès que la lecture démarre (URL réécrite en blob: entre-temps) ; ou totalement absente pour un morceau déjà en file avant la connexion du relais (déjà mis en cache localement, donc déjà en blob: au premier `buildRelayStateSnapshot()` vu par ce relais).
...
### 9.7 Recherche côté relais léger

- **SPEC-9.7.1** GIVEN un résultat de recherche dont l'artwork est déjà en cache serveur
  (l'API de recherche renvoie alors `artworkUrl`/`artUrl` sous forme de référence relative
  `/api/artwork?cachePath=...`, cf. `isLocal`/`cached` dans la réponse) — WHEN
  `_renderSearchResults()`, `_showActionSheet()` ou `_buildTrackPayload()` affichent ou
  transmettent cette artwork — THEN elle est d'abord normalisée en URL absolue via
  `resolveArtworkUrlForRelay(rawArt, API_BASE, API_TOKEN)` (`lib/downloaderConfig.js`),
  comme le reste de l'affichage relais (SPEC-9.3.4). Avant ce fix, ces trois points
  utilisaient l'artwork brute de l'API sans normalisation : une URL relative se résolvait
  contre l'origine de la page relais (pas l'API) et cassait l'image — visible uniquement
  pour les morceaux déjà disponibles localement sur le serveur (les résultats purement
  distants renvoient une URL iTunes/Deezer déjà absolue, donc non affectés).
- **SPEC-9.7.2** GIVEN un résultat de recherche marqué disponible localement sur le serveur
  (`isLocalTrackResult(track)`, `lib/searchUtils.js` — vrai si `track.cached`/`track.isLocal`
...
- **SPEC-20.5** BUG CORRIGÉ (29 juillet 2026) : la liste `ASSETS` de `sw.js` (précache installée à l'étape `install`) n'avait pas suivi l'extraction progressive de `main.js` vers `lib/*` — 26 fichiers `lib/*.js` en étaient absents, dont 22 activement importés par `main.js`/`player.js`/`relay.js` (ex. `blobStore.js`, `queueManager.js`, `trackStore.js`, `uiState.js`, `playbackController.js`, `relayModeManager.js`, …). Conséquence : ces modules n'étaient jamais mis en cache Service Worker, donc indisponibles hors-ligne après installation PWA (le prompt d'installation lui-même — SPEC-13.2.1 — restait fonctionnel, seul le fonctionnement hors-ligne était cassé). `ASSETS` liste désormais l'intégralité des fichiers `lib/*.js` (ordre alphabétique). Non-régression : `tests/unit/sw.test.js` compare dynamiquement le contenu de `dj-mix/lib/` à `ASSETS` et échoue si un fichier `lib/*.js` futur n'y est pas ajouté.
