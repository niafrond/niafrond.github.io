...
- **SPEC-1.3.8.17** GIVEN le curseur de mix par paliers (SPEC-1.3.8.16) atteignant `50%` de sa course (`computeBeatRepeatHalfStageIndex(N)` = premier index de palier `≥ N/2`) — THEN le deck **entrant** rejoint lui aussi l'état de bouclage : `DJPlayer#startBeatRepeatIncomingLoop` capture une fenêtre à la position courante du deck entrant (pas d'attente de grille rythmique comme SPEC-1.3.8.1 — piste encore inconnue de l'auditeur, un léger décalage y est nettement moins perceptible) et lance un second `BeatRepeatEngine` en réutilisant **les paliers restants et la même grille temporelle** que le deck sortant (`stageBeats.slice(halfIndex)`, même `secondsPerBeat`, `launchBeats` local = `launchBeats − computeBeatRepeatElapsedBeatsAtStageStart(stageBeats, halfIndex)`) — garantit que les deux platines atteignent la coupure finale (SPEC-1.3.8.10) exactement au même instant. Le `HTMLAudioElement` du deck entrant est coupé (SPEC-1.3.8.11) pendant cette fenêtre de fin uniquement, puis restauré au moment du saut final ou si la transition est interrompue avant que le second moteur n'ait eu le temps de démarrer (le décodage peut prendre plus de temps que la fenêtre restante ; dans ce cas le second moteur ne démarre simplement pas — pas de mute tardif ni de coupure de son manquée).
