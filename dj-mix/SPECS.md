...
- **SPEC-19.3.2** GIVEN Background Fetch indisponible, non configuré, OU un échec synchrone au dispatch — THEN une file interne à fenêtre glissante avec concurrence adaptative est utilisée (`computeNextBatchSize`, concurrence initiale `6`, plafond `20`, identique à SPEC-3.4.1/3.4.9). Chaque item passe par `pending` → `downloading` → `completed`/`failed`, écrit en IndexedDB à chaque transition en plus du statut en mémoire déjà affiché (`setFilRougeTrackStatus`/`renderTrackStatus`).
...
- **SPEC-19.3.4** `sw.js` transmet `id: bgFetch.id` (= l'identifiant du lot) dans les messages `BG_FETCH_DONE`/`BG_FETCH_FAIL` postés à la page. `filRougeDownloader.recordBackgroundFetchResult(id, succeededKeys, failedKeys, playlist)` marque les items correspondants `completed`/`failed` dans IndexedDB, puis retente les échecs et récupère les mix infos (SPEC-19.6.3/19.6.4) ; `recordBackgroundFetchFail(id)` (repli, pas de détail par item disponible) marque `failed` tout item encore `downloading` de ce lot.
...
### 19.6 Récupération partielle et retentatives avec backoff

Contexte : une seule réponse non-2xx (`/api/download` répond `404`/`500` par morceau raté) suffit à faire échouer TOUTE une registration Background Fetch (`failureReason: 'bad-status'`). Sans récupération, un gros lot finissait systématiquement « échoué » et les blobs déjà téléchargés étaient perdus.

- **SPEC-19.6.1** GIVEN l'événement `backgroundfetchfail` — THEN `sw.js` moissonne les enregistrements exactement comme `backgroundfetchsuccess` (helper partagé `_harvestBgFetchRecords` : blobs des réponses OK mis en Cache Storage, classement par clé `_ck`) et poste `BG_FETCH_DONE` avec `succeededKeys`/`failedKeys` (notification « Téléchargement interrompu : X mis en cache — Y à retenter »). `BG_FETCH_FAIL` n'est posté (repli SPEC-19.3.4) que si la moisson elle-même échoue ou ne produit aucune clé.
- **SPEC-19.6.2** GIVEN des morceaux en échec à l'issue d'un passage de la file interne — THEN ils sont retentés jusqu'à `MAX_DOWNLOAD_RETRY_ATTEMPTS` (= `3`) fois via la file interne, avec backoff exponentiel avant chaque vague : `DOWNLOAD_RETRY_BACKOFF_BASE_MS · 2^(n−1)` soit 2 s, 4 s, 8 s (constantes dans `lib/constants.js`, attente injectable via `waitFn` pour les tests). Chaque échec incrémente `retries` sur l'item (SPEC-19.1.5) ; un morceau récupéré passe `completed`. Les compteurs finaux du lot reflètent l'état post-retentatives.
- **SPEC-19.6.2.1** Avant chaque vague de retentatives, `apiHealthMonitor.recordSuccess()` est appelé pour réinitialiser le compteur de défaillances consécutives du moniteur de santé. Ceci évite que des échecs en lot (ex. démarrage à froid du serveur, timeout réseau transitoire) ne déclenchent le mode offline du moniteur, qui bloquerait immédiatement toutes les retentatives suivantes sans même tenter le réseau. Le lot de téléchargement a sa propre logique de retry avec backoff, indépendante du circuit-breaker du moniteur.
- **SPEC-19.6.3** GIVEN un `BG_FETCH_DONE` avec `failedKeys` non vide — THEN `recordBackgroundFetchResult(id, succeededKeys, failedKeys, playlist)` mappe les clés vers les morceaux du fil rouge courant et les retente selon la politique SPEC-19.6.2 (le lot repasse `running`, `transport: 'internal-queue'`). Les clés sans morceau correspondant restent `failed` (non retentables, même convention que SPEC-19.2.5). Toast final : « Retentatives terminées : X récupéré(s) [— Y échec(s)] ».
- **SPEC-19.6.4** Les mix infos des morceaux réussis via Background Fetch sont récupérées (`fetchMixData(name, artist)` → `hasMixInfo`) en parallèle des retentatives SPEC-19.6.3 — même comportement que la file interne après un téléchargement réussi (SPEC-3.5.x). Les morceaux récupérés par retentative obtiennent leur mix info via la file interne elle-même.
- **SPEC-19.6.5** GIVEN une expiration auth (`401`/`403`) pendant une vague de retentatives — THEN le comportement SPEC-19.4.2/19.4.3 s'applique (lot `paused-auth`, `onAuthExpired()`), et aucune vague supplémentaire n'est lancée.

### 19.7 Continuité écran éteint / application en arrière-plan

Contexte : Background Fetch (SPEC-19.3.1) est géré nativement par le navigateur et continue seul, écran éteint ou application en arrière-plan, sans JS de page actif. La file interne (repli, SPEC-19.3.2) dépend en revanche de `fetch()` exécutés depuis la page : si l'écran s'éteint (mise en veille), les navigateurs mobiles suspendent ou ralentissent fortement ce JS, ce qui bloquait un gros lot tant que l'utilisateur ne rallumait pas l'écran.

- **SPEC-19.7.1** `createDownloadBatchManager` accepte `onInternalQueueActiveChange(active: boolean)`, appelé `true` dès qu'un passage par la file interne démarre (premier passage ou retentatives, dans `startBatch`, `resumeIncompleteBatches` et `recordBackgroundFetchResult`) et `false` quand plus aucun passage n'est en cours (compteur de recouvrement, pas un simple booléen). Jamais appelé pour un lot entièrement pris en charge par Background Fetch. `main.js` câble cette raison sur un Wake Lock écran (`navigator.wakeLock`, raison `'download'`, partagée avec la raison `'playback'` déjà existante — l'écran reste allumé tant qu'au moins une raison est active).
