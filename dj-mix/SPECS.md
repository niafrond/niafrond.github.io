...
- 2026-08-15 : certaines vidéos (ex: chaîne Sing King) ont l'intégration désactivée par leur propriétaire — injouables dans `player.html`, et confirmé que le lecteur YouTube embarqué peut rester bloqué en boucle de retry interne sans jamais déclencher `onError`. Deux corrections : (1) `karaoke-playlist/index.html` appelle maintenant `videos.list?part=status` après chaque recherche pour exclure des résultats les vidéos avec `embeddable:false`, avant même qu'elles puissent être ajoutées à la file ; (2) `karaoke-playlist/player.html` ajoute un filet de sécurité `armStallTimer`/`STALL_TIMEOUT_MS` (15s) qui saute automatiquement une chanson si la lecture n'a jamais démarré, au cas où `onError` ne se déclenche pas.
