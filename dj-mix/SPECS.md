...
#### 1.3.5 Beat repeat synchronisé — RETIRÉ, remplacé par SPEC-1.3.8.15/.16/.5bis

- **SPEC-1.3.5.1 à .4 (retirées le 2026-07-28)** : `triggerBeatRepeatTransitionFx`/`triggerLoopRoll` (déclenché sur `transitionmode`, seek brut sur `HTMLAudioElement.currentTime` toutes les `windowMs`) faisait doublon avec le vrai moteur `BeatRepeatEngine` (SPEC-1.3.8) — et pire, comme il démarrait immédiatement sur la platine sortante alors que celle-ci n'est mute qu'après l'attente de l'ancrage (SPEC-1.3.8.1), c'était LUI qu'on entendait en premier : un bug perceptible comme « ça boucle sur une fraction de seconde au démarrage » plutôt que le vrai palier de départ (6 temps par défaut). Supprimé entièrement (`lib/djFxController.js`, plus d'appel depuis `main.js`) au profit du vrai moteur sur les deux platines (SPEC-1.3.8.17).
- `triggerLoopRoll` lui-même reste utilisé ailleurs (FX manuels ponctuels, `handleDjFxAction`) — seul l'appel automatique pour `beat_repeat` est retiré.
...
- **SPEC-1.3.8.1** GIVEN le mode `beat_repeat` — WHEN la transition démarre — THEN un point d'ancrage aligné sur la grille rythmique est calculé (`computeLoopAnchorSeconds`) : la position courante du deck sortant est arrondie au **prochain** temps (jamais en arrière — `secondsPerBeat` dérivé du BPM **du deck sortant**, `getSafeBeatRepeatBpm`, clampé `60`–`220`, défaut `120`), en supposant la phase zéro au début de la piste. Le moteur ne bascule sur cet ancrage qu'une fois que la lecture en direct du deck sortant l'a réellement atteint (`DJPlayer#waitForBeatRepeatAnchor`, scrutation de `context.from.currentTime`, en parallèle du décodage `BeatRepeatEngine#prepare`) — arrondir au temps le plus proche (potentiellement en arrière) puis démarrer le moteur immédiatement, comme dans une itération précédente de cette spec, faisait rejouer un instant déjà entendu : perceptible comme une saccade, pas comme un calage inaudible. **Limite assumée** : aucune détection de grille/downbeat réelle n'existe dans l'application (seul un BPM scalaire par piste), donc cet ancrage peut être mesurablement décalé sur une piste avec intro/pickup note — reste strictement meilleur que l'absence totale de calage.
- **SPEC-1.3.8.2** GIVEN l'ancrage calculé — THEN la longueur initiale du loop est déterminée par `chooseInitialLoopBeats(remainingBeats, desiredInitialBeats)` : `desiredInitialBeats` = `player.beatRepeatInitialBeats` (configurable, défaut `BEAT_REPEAT_DEFAULT_INITIAL_BEATS` = `6` temps) ; si moins de `6` temps restent avant la fin de la piste sortante (`remainingBeats = (duration − currentTime) / secondsPerBeat` du deck sortant), la longueur initiale retombe à `3` temps (`BEAT_REPEAT_FALLBACK_INITIAL_BEATS`) ; jamais moins de `2` temps (`BEAT_REPEAT_MIN_INITIAL_BEATS`).
- **SPEC-1.3.8.3** GIVEN la longueur initiale choisie — THEN elle est divisée par 2 successivement jusqu'à un plancher **absolu** de `1/16` temps (`BEAT_REPEAT_FLOOR_BEATS`, indépendant de la longueur initiale — `buildBeatRepeatStageBeats`) : pour `6` temps initiaux (défaut), la séquence est `6 → 3 → 3/2 → 3/4 → 3/8 → 3/16 → 3/32 → 1/16` (8 paliers) ; pour `3` temps (repli), `3 → 3/2 → 3/4 → 3/8 → 3/16 → 3/32 → 1/16` (7 paliers) ; pour `2` temps, `2 → 1 → 1/2 → 1/4 → 1/8 → 1/16` (6 paliers) — une longueur initiale plus courte produit moins de paliers, pas une séquence rééchelonnée.
...
- **SPEC-1.3.8.9** GIVEN les `BEAT_REPEAT_FX_STAGE_COUNT` = `2` derniers paliers (les plus courts) — THEN un filtre high-pass est appliqué sur le deck sortant (`deckFx.filterMode = 'highPass'`) et l'écho est activé avec une intensité qui croît linéairement de `0` à `1` sur la durée cumulée de ces 2 paliers (`SimpleMixFeatures#setEchoIntensity`, ramp continu sans re-seek du stem audio) — ni le filtre ni l'écho ne sont actifs pendant les paliers précédents. Ces deux effets sont explicitement optionnels dans la spécification d'origine ; ils restent implémentés ici car peu coûteux et cohérents avec la « tension rythmique croissante » recherchée.
- **SPEC-1.3.8.10** GIVEN la limite de mesure atteinte — THEN le deck entrant passe au volume plein par un **saut instantané** (pas un fondu) : `fromBase = 0`, `toBase = 1` appliqués dans le même tick — cohérent avec l'esthétique « par saccades » du reste de l'effet plutôt qu'un lissage de dernière seconde ; le moteur `BeatRepeatEngine` du deck sortant s'arrête indépendamment (son propre fondu de sortie, géré par SPEC-1.3.8.7/1.3.8.8, a déjà ramené son gain à `0` avant cet instant).
- **SPEC-1.3.8.11** Pendant sa propre phase bouclée, la contribution du `HTMLAudioElement` de la platine concernée au graphe Web Audio est coupée (`SimpleMixFeatures#setDeckElementGain(deck, 0)`, ramp `ELEMENT_MUTE_RAMP_SEC` = `8 ms`) pour que seul le moteur `BeatRepeatEngine` de cette platine soit audible, évitant toute double lecture ; elle est restaurée (`= 1`) une fois SA boucle terminée ou la transition interrompue. S'applique au deck sortant pendant toute la phase, et au deck entrant uniquement pendant sa fenêtre de fin (SPEC-1.3.8.17).
...
- **SPEC-1.3.8.13** La durée totale de `beat_repeat` n'est pas dérivée de la durée de crossfade configurée par l'utilisateur : elle est entièrement dérivée du BPM du deck sortant et de la grille de mesures (`computeBeatRepeatLoopPhaseMs(bpm, initialBeats)`) ; le saut final de SPEC-1.3.8.10 est instantané et n'ajoute donc plus de fenêtre de superposition à cette durée.
- **SPEC-1.3.8.14 (retirée le 2026-07-28)** — voir SPEC-1.3.5 : le FX décoratif `triggerBeatRepeatTransitionFx` qu'elle documentait a été supprimé, remplacé par le vrai moteur sur les deux platines (SPEC-1.3.8.17).
- **SPEC-1.3.8.15** GIVEN le dernier palier (plancher `1/16` temps) — THEN son `AudioBufferSourceNode` subit un glissement de tempo (`playbackRate`, `computeBeatRepeatFinalStagePitchRatio(fromBpm, toBpm) = getSafeBeatRepeatBpm(toBpm) / getSafeBeatRepeatBpm(fromBpm)`) qui part de `1` (tempo natif du deck sortant) au début du palier et rampe linéairement jusqu'au ratio cible à l'instant du lancement — objectif « frictionless » : les derniers bégaiements glissent vers le tempo du morceau B au lieu de s'arrêter net sur un tempo différent. Pas d'alignement de phase avec un downbeat de la piste B (même limitation que SPEC-1.3.8.1 : aucune détection de grille sur la piste entrante) — seul le tempo est rapproché, pas la phase.
- **SPEC-1.3.8.16** Le curseur de mix (volumes `fromBase`/`toBase`) avance **par paliers** pendant toute la phase bouclée, et non en fondu continu : à chaque changement de palier, il saute d'exactement `1/N` de sa course totale vers le deck entrant (`N` = nombre de paliers, `computeBeatRepeatStepProgress`/`computeBeatRepeatStageIndexAtElapsedBeats`), en réutilisant la courbe linéaire standard (`#computeTransitionLevels`) évaluée à ce `t` par paliers plutôt qu'à un `t` continu. Le dernier `1/N` de la course est le saut instantané de SPEC-1.3.8.10 à la limite de mesure — le curseur atteint donc `1` en `N` sauts au total, jamais par une rampe.
- **SPEC-1.3.8.17** GIVEN le palier à `50%` de la course du curseur (`computeBeatRepeatHalfStageIndex(N)` = premier index de palier `≥ N/2`) — THEN le deck **entrant** est réellement **dans le même état de bouclage** que le deck sortant à partir de cet instant et jusqu'à la coupure finale, pas seulement « également en train de boucler sur un calendrier voisin » : `DJPlayer#runBeatRepeatTransition` calcule l'instant exact (`startAt = t0 + computeBeatRepeatElapsedBeatsAtStageStart(stageBeats, halfIndex) × secondsPerBeat`, où `t0` est l'instant `AudioContext` réellement utilisé par le moteur du deck sortant, retourné par `BeatRepeatEngine#run`) et lance dès le début de la phase bouclée (pas de manière réactive à un `tick`) le décodage d'un second `BeatRepeatEngine` sur le deck entrant, programmé pour démarrer pile à cet instant via `BeatRepeatEngine#run({ ..., startAt })` — même mécanisme de programmation à l'avance contre l'horloge `AudioContext` que SPEC-1.3.8.6, pas une vérification réactive dans la boucle de `tick` (qui aurait laissé la latence du décodage désynchroniser les deux platines). Le second moteur réutilise **les paliers restants et la même grille temporelle** que le deck sortant (`stageBeats.slice(halfIndex)`, même `secondsPerBeat`, `launchBeats` local = `launchBeats − computeBeatRepeatElapsedBeatsAtStageStart(stageBeats, halfIndex)`), garantissant que les deux platines atteignent la coupure finale (SPEC-1.3.8.10) exactement au même instant. Le `HTMLAudioElement` du deck entrant est coupé (SPEC-1.3.8.11) via un `setTimeout` calé sur ce même `startAt` (pas plus tôt — silence — pas plus tard — double lecture), puis restauré au moment du saut final ; si la transition se termine avant que ce `setTimeout` ne se déclenche, il est annulé et résolu immédiatement plutôt que de laisser le nettoyage final attendre son propre délai. Pas d'attente de grille rythmique sur la position de capture elle-même (contrairement à SPEC-1.3.8.1) : piste encore inconnue de l'auditeur, un léger décalage de contenu y est nettement moins perceptible que sur le deck sortant — seul l'**instant de démarrage** doit être exact, pas le point d'ancrage dans la piste.
