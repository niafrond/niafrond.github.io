...
- **SPEC-2.5.6** GIVEN ce bouton est utilisé pour forcer un re-téléchargement/rafraîchissement du morceau courant de file — THEN le flux ignore explicitement les raccourcis locaux (`cachePath`, blob en mémoire, blob persistant IndexedDB, `persistedSourceUrl`, lookup `trackPathDb`) le temps de l'action, afin de forcer une résolution API fraîche (`POST /api/download`) au lieu de rejouer une source potentiellement obsolète.
