...
describe('SPEC-1.3.8.2 — chooseInitialLoopBeats', () => {
  test('defaults to 8 beats when plenty of runway remains', () => {
    expect(BEAT_REPEAT_DEFAULT_INITIAL_BEATS).toBe(8);
    expect(chooseInitialLoopBeats(100, 8)).toBe(8);
  });

  test('falls back to 4 beats when fewer than 8 beats remain', () => {
    expect(BEAT_REPEAT_FALLBACK_INITIAL_BEATS).toBe(4);
...
describe('SPEC-1.3.8.3 — buildBeatRepeatStageBeats', () => {
  ...
  test('defaults to the 8-beat sequence with no arguments', () => {
    expect(buildBeatRepeatStageBeats()).toEqual(buildBeatRepeatStageBeats(8));
  });

  test('falls back to the default for invalid initialBeats', () => {
    expect(buildBeatRepeatStageBeats(0)).toEqual(buildBeatRepeatStageBeats(8));
    expect(buildBeatRepeatStageBeats(-4)).toEqual(buildBeatRepeatStageBeats(8));
    expect(buildBeatRepeatStageBeats(NaN)).toEqual(buildBeatRepeatStageBeats(8));
  });
});
...
describe('SPEC-1.3.8.4/.5 — computeBeatRepeatLaunchBeats', () => {
  test('default 8-beat sequence: total is 32 beats (8 bars)', () => {
...
describe('SPEC-1.3.8.1 — computeLoopAnchorSeconds', () => {
  test('snaps to the nearest beat boundary (phase-zero at track start)', () => {
    expect(computeLoopAnchorSeconds(0, 0.5)).toBeCloseTo(0, 6);
    expect(computeLoopAnchorSeconds(0.24, 0.5)).toBeCloseTo(0, 6);
...
describe('SPEC-1.3.8.13 — computeBeatRepeatLoopPhaseMs', () => {
  test('default 8-beat loop at 120 BPM totals 16000ms (32 beats)', () => {
    expect(computeBeatRepeatLoopPhaseMs(120)).toBeCloseTo(16000, 5);
  });
