 ...
 * @property {Object} [nextItem]   — item at transition.toItemId (fil rouge playlist or queue)
 */
 ...
 *
 * @param {{ enabled: boolean, playlist: Object[], queue?: Object[], playingId: string|number|null, currentIndex: number }} opts
 * @returns {DjPlanIndicatorState}
 */
export function computeDjPlanIndicatorState({ enabled, playlist, queue = [], playingId, currentIndex }) {
  ...
  // toItemId peut désigner un morceau de la file d'attente qui n'appartient pas
  // (ou plus) au fil rouge (cf. djPlanManager.planCurrentToNextTransition, qui
  // préfère l'ordre réel de la file d'attente) : on cherche donc aussi dedans.
  const nextItem = playlist.find((p) => String(p.id) === String(t.toItemId))
    ?? queue.find((q) => String(q.id) === String(t.toItemId))
    ?? null;
