/**
 * sound.js — Sons synthétiques pour le jeu Pyramide
 *
...
export function playBuzzer() {
  ...
  try {
    ...
    [0, 0.5, 1.0].forEach(offset => {
      ...
      osc.frequency.linearRampToValueAtTime(110, now + offset + 0.4);
      g.gain.setValueAtTime(0.4, now + offset);
      g.gain.linearRampToValueAtTime(0, now + offset + 0.4);
...
export function playLevelUp() {
  ...
  try {
    ...
    notes.forEach((freq, i) => playNote(freq, 'triangle', now + i * 0.1, 0.2, 0.35));
  } catch (_) {}
}
...
/** Pyramide complétée — mélodie triomphale */
export function playPyramidComplete() {
  if (_muted) return;
  try {
    const ctx = getCtx();
    const now = ctx.currentTime;
    const melody = [
      [523.25, 0.1], [659.25, 0.1], [783.99, 0.1], [1046.5, 0.1],
      [783.99, 0.08], [880, 0.08], [1046.5, 0.3],
    ];
    let t = now;
    melody.forEach(([freq, dur]) => {
      playNote(freq, 'triangle', t, dur + 0.04, 0.4);
      t += dur + 0.02;
    });
  } catch (_) {}
}
