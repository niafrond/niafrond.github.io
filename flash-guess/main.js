...
const GROUPS_KEY                 = 'flashguess_groups';
...
// ─── GROUPES : affichage dans le setup ────────────────────────────────────────
function renderGroupsInSetup() {
...
function renderGroupsEditor() {
  ...
  groups.forEach((group, groupIdx) => {
    ...
    if (group.members.length === 0) {
      ...
      group.members.forEach((name, memberIdx) => {
        const memberData = allMembers.find(m => m.name === name);
        const isChild = memberData?.isChild ?? false;
